...
The code used for the computations is the same as that used in \citet{Kruschke2013BEST}.
...
The default priors are minimally informative: normal priors with large standard deviation for ($\mu$), broad uniform priors for ($\sigma$), and a shifted-exponential prior for ($\nu$), as shown in the upper part of Figure~\ref{fig:model}. Full details of the default priors are given in \citet{Kruschke2013BEST}. See the help page for \verb@BESTmcmc@ for instructions on setting informative priors.

For a general discussion see chapters 11 and 12 of \citet{Kruschke2011book}.
...
BEST also requires the packages \verb@jagsUI@ and \verb@coda@, which should normally be installed at the same time as package BEST if you use the \verb@install.packages@ function in \R{}.
...
BESTout1g <- BESTmcmc(y0, parallel=FALSE)
@
\begin{verbatim}
Processing function input....... 
...
The default is to use uninformative priors as described in Kruschke (2013), but \code{BESTmcmc} allows you to specify your own informative priors. See the help page.
...
If you want to know how the functions in the \code{BEST} package work, you can download the \R{} source code from CRAN or from GitHub \url{https://github.com/mikemeredith/BEST} or find almost the same code at \url{http://www.indiana.edu/~kruschke/BEST/} together with links to articles, videos, and the blog.
